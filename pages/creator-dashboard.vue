...
div.pa-10
  ...
  div.d-flex.flex-row(v-if="sold.length"  style="max-width:1024px; width:100%; gap:20px")
    v-card.mb-6(v-for="(soldNft, index) in sold" :key="index" style="max-width:300px; width:100%")
      div.overflow-hidden.d-flex.align-center.justify-center(style="background-color: white; height:300px")
        img(:src="soldNft.image ? soldNft.image : 'https://via.placeholder.com/350'" alt="NFT" style="width:100%")
      v-card-title {{soldNft.name}}
      v-card-text
        p {{soldNft.description}}
      v-card-actions.d-flex.justify-end.align-center.pa-4.pt-0
        p.mb-0.mr-3 {{soldNft.price}} ETH
